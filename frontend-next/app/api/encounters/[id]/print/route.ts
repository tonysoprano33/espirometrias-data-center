...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: files, error } = await supabase.from("attachments").select("storage_bucket, object_path, file_kind, created_at").eq("encounter_id", id).in("file_kind", ["informe_pdf", "pdf_resultado"]).order("file_kind", { ascending: true }).order("created_at", { ascending: false }).limit(1);
