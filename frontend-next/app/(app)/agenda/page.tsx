...
export default async function AgendaPage() {
  ...
  const [{ data, error }, { data: physicianData }] = await Promise.all([
    supabase.rpc("agenda_entries_v2", { target_date: today }),
  ...
  return <main className="shell">
    <section className={`card agenda-card ${profile.role === "secretaria" ? "secretary-agenda" : "espirometrista-agenda"}`}>
      ...
      <section className={`agenda-work-grid ${profile.role === "secretaria" ? "is-secretary" : "is-operator"}`} aria-label="Pacientes del dia">
        <div className="agenda-work-head"><span>Hora</span><span>Paciente</span><span>DNI</span><span>Estudio</span><span>Cobertura</span>{profile.role !== "secretaria" && <><span>Dr. deriva</span><span>SO2 / FC reposo</span><span>SO2 / FC post</span><span>Resultado</span></>}<span>Asistencia</span>{profile.role !== "secretaria" && <span>Acciones</span>}</div>
