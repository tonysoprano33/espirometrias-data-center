...
export default async function MedicalReviewDetailPage({ params }: DetailPageProps) {
  ...
  const attachments = (encounter.attachments ?? []) as Array<{ original_name: string; file_kind: string; storage_bucket: string; object_path: string; analysis_status: string }>;
  ...
  return <main className="shell">
    ...
    <div className="review-detail-grid">
      <section className="review-document">
        <div className="document-status"><b>{source ? source.original_name : "Sin archivo original"}</b><span>{source ? `Archivo ${source.analysis_status}` : "La revision queda pendiente de PDF o foto"}</span></div>
